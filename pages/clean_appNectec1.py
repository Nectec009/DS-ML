...
st.set_page_config(layout="wide", page_title="Data Cleaning Workshop App", initial_sidebar_state="expanded")

# --- Custom CSS for Apple-like Blue-Red Theme with Transparency ---
st.markdown(
    """
    <style>
    :root {
        --primary-blue: #007AFF; /* A bright, Apple-like blue */
        --secondary-red: #FF3B30; /* A vibrant, Apple-like red */
        --background-gradient-start: #e0f2f7; /* Light Sky Blue */
        --background-gradient-end: #ffe0e6; /* Light Pink */
    ...
    /* Main Streamlit App container to allow body gradient to show */
    .stApp {
    ...
    .main .block-container {
        ...
        padding-left: 1rem;
        padding-bottom: 2rem;
        background-color: var(--card-background); /* Transparent white for content blocks */
    ...
    /* Headers */
    h1, h2, h3, h4, h5, h6 {
        color: var(--primary-blue); /* Use blue for headers */
        font-weight: 700;
    ...
    .stSidebar > div:first-child {
        background-color: rgba(255, 255, 255, 0.85); /* Slightly less transparent white for sidebar */
        border-right: 1px solid var(--border-color);
        backdrop-filter: blur(15px); /* More blur for sidebar */
        border-radius: 0 12px 12px 0; /* Rounded only on the outer edge */
    ...
    .stButton>button:hover {
        background-color: var(--secondary-red); /* Hover effect to red */
    ...
    .stAlert {
        border-radius: 10px;
        background-color: var(--card-background); /* Use transparent card background for alerts */
    ...
    .stAlert.st-error {
        border-left-color: var(--secondary-red); /* Red */
    }
...
if uploaded_file is not None: # ถ้ามีการอัปโหลดไฟล์แล้ว
    ...
    def perform_data_ranking(data): # ฟังก์ชันสำหรับจัดอันดับข้อมูล
        st.subheader("🏆 7. Data Ranking")
        st.write("##### เลือกคอลัมน์สำหรับจัดอันดับ")

        # Identify numerical columns for ranking value
        numerical_cols = data.select_dtypes(include=[np.number]).columns.tolist()
        # Identify categorical columns for grouping
        categorical_cols = data.select_dtypes(include=['object', 'category']).columns.tolist()

        if 'Date' in categorical_cols:
            categorical_cols.remove('Date') # Remove Date if it was mistakenly classified as object

        if not numerical_cols or not categorical_cols:
            st.warning("ไม่พบทั้งคอลัมน์ตัวเลขและคอลัมน์หมวดหมู่ที่เหมาะสมสำหรับการจัดอันดับ.")
            return data

        col_to_rank = st.selectbox("เลือกคอลัมน์ตัวเลขที่ต้องการจัดอันดับ (เช่น Units_Sold, Marketing_Spend)", numerical_cols)
        col_to_group = st.selectbox("เลือกคอลัมน์หมวดหมู่เพื่อจัดกลุ่ม (เช่น Region, Product_Variant, Channel)", categorical_cols)

        if col_to_rank and col_to_group:
            st.write(f"##### จัดอันดับ '{col_to_rank}' ตามกลุ่ม '{col_to_group}'")
            
            # Calculate sum of the numerical column for each group
            ranked_data = data.groupby(col_to_group)[col_to_rank].sum().sort_values(ascending=False).reset_index()
            ranked_data['Rank'] = ranked_data[col_to_rank].rank(method='dense', ascending=False).astype(int)
            st.dataframe(ranked_data)

            st.write("##### แสดง Top 10 ของการจัดอันดับ")
            st.dataframe(ranked_data.head(10))

            st.write("##### กราฟแสดง Top 10")
            fig, ax = plt.subplots(figsize=(10, 6))
            sns.barplot(x=col_to_rank, y=col_to_group, data=ranked_data.head(10), ax=ax)
            ax.set_title(f'Top 10 Ranked {col_to_rank} by {col_to_group}')
            st.pyplot(fig)
            plt.close(fig)
        else:
            st.info("กรุณาเลือกคอลัมน์ตัวเลขและคอลัมน์หมวดหมู่เพื่อทำการจัดอันดับ.")
    ...
    do_ranking = st.sidebar.checkbox("7. Data Ranking", value=True) # Checkbox สำหรับ Data Ranking
    ...
    if st.button("Start Cleaning"): # ปุ่มสำหรับเริ่มกระบวนการ Data Cleaning
        ...
        if do_ranking: # ถ้าเลือก Data Ranking
            df = perform_data_ranking(df)

        st.markdown("---  ") # เส้นแบ่ง
        st.subheader("✅ Final Cleaned Data Summary") # หัวข้อสรุปผล
        ...
        st.download_button( # ปุ่มสำหรับดาวน์โหลดข้อมูล
            ...
            file_name="Clean Nectec.csv", # ชื่อไฟล์เมื่อดาวน์โหลด (Changed from redbull_clean.csv)
...
if st.button("🏠 กลับหน้าหลัก"): # New button added based on previous code
    # This assumes 'app.py' exists. If not, this will cause an error.
    # For a single-page app, this might not be needed or would link to itself.
    # If 'app.py' is a different Streamlit app, then st.switch_page is correct.
    # As there is no 'app.py' in the provided context, I will comment this out
    # or remove it if it causes issues. For now, assuming it's part of a multi-page setup.
    try:
        st.switch_page("app.py")
    except Exception as e:
        st.warning(f"Could not switch page: {e}. 'app.py' might not exist or be configured.")
